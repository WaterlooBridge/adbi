...
int main(int argc, char *argv[])
{
	...
	*/
	...
	regs.ARM_r0 = sc[18]; // want to make stack executable
	//printf("r0 %x\n", regs.ARM_r0);
	regs.ARM_r1 = RTLD_NOW| RTLD_GLOBAL; // stack size
	...
	if (nomprotect == 0) {
		...
		regs.ARM_lr = 0; // points to loading and fixing code
		regs.ARM_pc = sc[19]; // execute mprotect()
	}
